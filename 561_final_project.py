...
#scoring matrix for Needleman-Wunsch
...
#read in reference sequence and confidence values for reference sequence
def read_files():
    global confList
    global sequence
    global seq_arr
...
#randomly generate a query from the reference sequence (for testing purposes)
def generate_query(length,matrix):
    query=""
    len_i_d = 10 #max length of inserts and deletions
    start = random.randrange(len(matrix)-length-1)
    end = start+length
    #print(start,end)
    arr_nuc = ['A','C','G','T']
    nb_ins = 0
...
if __name__ == '__main__':
    ...
    query_length = sys.argv[1]
    read_files()
    #matrix = make_matrix()[:][0:10000]
    matrix = make_matrix()
